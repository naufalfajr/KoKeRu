...
function loadDynamicContentModal(idruang) {
  $("#demo-modal").load("functions/popup.php?idruang=" + idruang, function () {
    $("#modal-mdl").modal({
      show: true,
    });
  });
...
function loadLightBox(bukti) {
  if (getExtension(bukti) == "mp4") {
    $("#light-box-vid").removeClass('d-none');
    $("#light-box-vid").attr("src", "assets/img/" + bukti);
    $("#light-box-img").addClass('d-none');
  } else {
    $("#light-box-img").removeClass('d-none');
    $("#light-box-img").attr("src", "assets/img/" + bukti);
    $("#light-box-vid").addClass('d-none');
  }
